...
try:
    ...
    print("FATAL ERROR: 'epg/channel-map.json' not found. Using empty map.")
    channel_map = {}
except json.JSONDecodeError:
    print("FATAL ERROR: 'epg/channel-map.json' is not valid JSON. Using empty map.")
    channel_map = {}
...
def parse_time_range(date_str_slash, time_range_str):
    """
    Parses time range strings like '00:00 - 01:30' with a date '2025/11/26'.
    Handles cross-day programs (if end time is earlier than start time, it assumes next day).
    """
    try:
        ...
        print(f"[警告] 時間範圍解析失敗 '{time_range_str}' on {date_str_slash}: {e}")
        return None, None

def fmt(dt):
    """Formats a datetime object or a string into the XMLTV required time format."""
...
def parse_xmltv_time(timestr):
    """Parses an XMLTV time string back into a datetime object."""
...
def p_time(p):
    """Helper to get the program start time as a datetime object."""
...
def write_xml(root_element, filepath):
    """Writes the ElementTree to a file with UTF-8 encoding."""
    indent(root_element)
...
def write_channel_and_programmes(xml_root, ch_id, ch_name, programmes, with_desc=True):
    """Adds channel definition and all programmes to the XML root."""
    ...
    for p in sorted(programmes, key=lambda x: p_time(x)):
        prog_el = SubElement(xml_root, "programme",
                             ...
                             stop=fmt(p['end']),
                             channel=ch_name)
        SubElement(prog_el, "title", lang="zh").text = p['title']
        if with_desc and p.get('desc'):
            SubElement(prog_el, "desc", lang="zh").text = p['desc']
        elif not with_desc:
            SubElement(prog_el, "desc", lang="zh").text = ""

def fetch_api_programmes(channels_api, channel_map, date_str_api, yesterday_str_api):
    """Fetches EPG data from the external EPG API (epg.pw)."""
    
    def fetch_epg(channel_id, date_str=None):
        """Helper to fetch raw XML data."""
    ...
    def parse_epg(xml, date_prefix, mode='today', channel_id=None):
        """
        Parses XMLTV data.
        mode='today' (programs starting today)
        mode='carry' (programs ending today that started yesterday/earlier)
        """
        try:
        ...
        for prog in root.findall('programme'):
            ...
            try:
                start_dt = datetime.strptime(start_raw, "%Y%m%d%H%M%S %z").astimezone(timezone(timedelta(hours=8))).replace(tzinfo=None)
                stop_dt = datetime.strptime(stop_raw, "%Y%m%d%H%M%S %z").astimezone(timezone(timedelta(hours=8))).replace(tzinfo=None)
            except Exception as e:
                continue
    ...
    for name in channels_api:
        ...
        if not real_id:
            print(f"[警告] 頻道 '{name}' 在 channel-map 中未找到對應 ID。")
            continue
        ...
        try:
            ...
            if today_programmes:
                ...
                if first_start.time() != datetime.min.time():
                    if carryover_programmes:
                        last_prog = carryover_programmes[-1]
                        
                        carry_start = datetime.combine(first_start.date(), datetime.min.time())
            ...
            print(f"[錯誤] {name} HTTP 請求失敗 ({real_id})：{e}")
        except Exception as e:
            print(f"[錯誤] {name} 抓取或解析失敗 ({real_id})：{e}")

    return epg_programmes

def fetch_ottltv_programmes():
    """Fetches EPG data by scraping LTV OTT website."""
    url = "https://www.ltv.com.tw/ott%e7%af%80%e7%9b%ae%e8%a1%a8/"
    all_programmes = {cid: [] for cid in channels_ottltv}
    ...
    for cid in channels_ottltv:
        ...
        for item in items:
            ...
            popup_a = item.select_one("a")
            
            if not title_tag or not time_tag or not popup_a:
                continue
            
            popup_href = popup_a.get("href")
            if not popup_href:
                continue
...
def fetch_modltv_programmes():
    """Fetches EPG data by scraping LTV MOD website (logic similar to OTT)."""
    url = "https://www.ltv.com.tw/mod%e7%af%80%e7%9b%ae%e8%a1%a8/"
    all_programmes = {cid: [] for cid in channels_modltv}
...
def fetch_json_schedule():
    """Fetches EPG data from custom JSON endpoint (e.g., Meiya Movie HD)."""
    programmes = []
    
    for ch_id, info in channels_json.items():
        try:
            data = requests.get(info['url'], timeout=15).json()          
            ...
            print(f"[錯誤] 無法抓取或連接 {ch_id} ({info['url']})：{e}")
        except Exception as e:
            print(f"[錯誤] 解析 {ch_id} JSON 失敗：{e}")

    programmes.sort(key=lambda x: (x['channel'], x['start']))
    return programmes

def fetch_ls_time_programmes():
    """Fetches EPG data by scraping LS-Time website (JSON embedded in HTML)."""
    ...
    try:
        ...
        print(f"[錯誤] 抓取 LS-Time 失敗 (HTTP/Connect)：{e}")
        return None
    except Exception as e:
        print(f"[錯誤] 抓取 LS-Time 失敗 (解析)：{e}")
        return None

def fetch_celestial_programmes():
    """Fetches EPG data by scraping Celestial Movies website."""
    
    channels = [
        ("天映頻道", channels_celestial["celestial-movies-hd"]["url"], "celestial-movies-hd"),
    ]

    celestial_by_channel = {}

    now_date = datetime.now(timezone.utc) + timedelta(hours=8)
    ...
    for name, url, ch_id in channels:
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                if i + 1 < len(items):
                    next_time_tag = items[i + 1].select_one(".schedule-time")
                    if next_time_tag:
                        next_time_str = next_time_tag.get_text(strip=True)
                        try:
                ...
                if end.date() > start.date():
                    ...
                    programmes.append({
                        "channel": ch_id, "start": start, "end": midnight,
                        "title": title, "desc": desc, "name": name,
                    })
                    programmes.append({
                        "channel": ch_id, "start": midnight, "end": end,
                        "title": title, "desc": desc, "name": name,
                    })
                else:
                    programmes.append({
                        "channel": ch_id, "start": start, "end": end,
                        "title": title, "desc": desc, "name": name,
                    })
            ...
            if programmes:
                ...
                if first_start > day_start:
                    programmes.insert(0, {
                        "channel": ch_id, "start": day_start, "end": first_start,
                        "title": "無節目資料", "desc": "", "name": name,
                    })

            celestial_by_channel[ch_id] = programmes

        except requests.exceptions.RequestException as e:
            print(f"[錯誤] 抓取 {name} 失敗 (HTTP/Connect)：{e}")
            celestial_by_channel[ch_id] = []
        except Exception as e:
            print(f"[錯誤] 抓取 {name} 失敗 (解析)：{e}")
            celestial_by_channel[ch_id] = []

    return celestial_by_channel

def main():
    """Main function to fetch all EPG data and write the XML files."""
    print("--- 1. Fetching EPG data from sources ---")
    epg_programmes = fetch_api_programmes(channels_api, channel_map, date_str_api, yesterday_str_api)
    print(f"  > API Programmes fetched: {len(epg_programmes)}")
    
    ottltv_programmes = fetch_ottltv_programmes()
    print(f"  > LTV OTT Programmes fetched: {sum(len(v) for v in ottltv_programmes.values())}")
    
    modltv_programmes = fetch_modltv_programmes()
    print(f"  > LTV MOD Programmes fetched: {sum(len(v) for v in modltv_programmes.values())}")
    
    json_programmes = fetch_json_schedule()
    print(f"  > Custom JSON Programmes fetched: {len(json_programmes)}")
    
    ls_time_data = fetch_ls_time_programmes()
    ls_time_programmes = ls_time_data['programmes'] if ls_time_data else []
    print(f"  > LS-Time Programmes fetched: {len(ls_time_programmes)}")

    celestial_programmes_map = fetch_celestial_programmes()
    celestial_programmes = celestial_programmes_map.get('celestial-movies-hd', [])
    print(f"  > Celestial Programmes fetched: {len(celestial_programmes)}")


    tv_epg = Element("tv", {'source-info-name': 'Custom EPG'})
    tv_boss = Element("tv", {'source-info-name': 'Boss EPG'})
    ...
    if ls_time_data:
        all_channels.append((ls_time_data['id'], ls_time_data['name'], ls_time_programmes, True))

    celestial_ch_id = "celestial-movies-hd"
    celestial_ch_name = channels_celestial[celestial_ch_id]["name"]
    if celestial_programmes:
        all_channels.append((celestial_ch_id, celestial_ch_name, celestial_programmes, True))

    print("\n--- 2. Generating epg.xml ---")
    ...
    print("  > Successfully wrote epg.xml")

    print("\n--- 3. Generating boss.xml (Subset) ---")
    boss_subset_ids = list(channels_ottltv.keys()) + list(channels_modltv.keys()) + list(channels_json.keys()) + ["LS-Time", celestial_ch_id]

    for ch_id, ch_name, programmes, with_desc in all_channels:
        if ch_id in boss_subset_ids:
            write_channel_and_programmes(tv_boss, ch_id, ch_name, programmes, with_desc)

    write_xml(tv_boss, "boss.xml")
    print("  > Successfully wrote boss.xml")
