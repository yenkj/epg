...
def fetch_ottltv_programmes():
    ...
    for cid, channel_name in channels_ottltv.items():
        all_programmes[cid] = []
        ...
        for item in items:
            ...
            try:
                ...
                start_epg = f"{date_part} {start_time}"
                end_epg = f"{date_part} {end_time}"
            except Exception as e:
                print(f"Error parsing time range: {e}")
                start_epg, end_epg = None, None
            if start_epg and end_epg:
                if any(start_epg.startswith(day) for day in three_day_list):
