...
const RAW_EPG_XML = "https://raw.githubusercontent.com/yenkj/epg/main/epg.xml";
...
function parseEPG(xml, dayPrefix, mode = "today") {
  ...
  while ((match = programmeRegex.exec(xml))) {
    ...
    if (mode === "carry" && endRaw.startsWith(dayPrefix)) {
...
export default {
  async fetch(request) {
    ...
    // === epg.xml 重定向 ===
    if (pathname === "/epg.xml") {
      return Response.redirect(RAW_EPG_XML, 302);
    }
    ...
    try {
      ...
      if (realId) {
        ...
        epg_data = [...carryOverList, ...todayList];
      }
