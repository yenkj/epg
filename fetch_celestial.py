...
def write_xmltv(programmes, channels, output_path):
    ...
    # 再按照频道顺序 + 节目时间顺序添加 programme
    for ch in channels:
        channel_id = ch["id"]
        channel_programmes = [p for p in programmes if p["channel"] == channel_id]
        for prog in sorted(channel_programmes, key=lambda x: x["start"]):
...
if __name__ == "__main__":
    ...
    all_programmes = []
    for ch in channels:
        print(f"Fetching: {ch['url']} → {ch['id']}")
        result = fetch_celestial_generic(ch["name"], ch["url"], ch["id"])
